...
int main(int argc, char **argv) {
    ...
    printf("ht1_data=%p\n", get_ht_data(get_JPEG_ht(jpeg)[2])); // table_ht 10 >>> index 2 !!!!!!
    ...
    unsigned char *p_ht0_data = get_ht_data(get_JPEG_ht(jpeg)[0]);
    unsigned char *p_ht2_data = get_ht_data(get_JPEG_ht(jpeg)[2]);
    unsigned char *p_jpeg_data = get_JPEG_data(jpeg);
    size_t jpeg_data_size = get_JPEG_data_size(jpeg);
    // ok jusqu'ici
